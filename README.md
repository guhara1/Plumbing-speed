...
> 전국 17개 시·도 → 시·군·구 → (행정구) → 행정동/읍·면까지 계층형으로 커버합니다.
> 서울·6대 광역시는 전 행정동, 경기·8개 도는 시·군 전체 + 거점 시/군 읍·면·동까지.
> 표의 수치는 확장 진행에 따라 계속 늘어납니다.
...
| 서울 행정동(동네) | 249 | `/area/seoul/<자치구>/<동>/` (25개 구 전 행정동, 번호 동은 대표 1개) |
